...
open (OUT, ">/group/ober-resources/users/smozaffari/Prediction/results/fevparents_corrected") || die "nope: $!";
...
#foreach my $d (sort keys %family) {
	...
	#my $ind = $family{$d}{$m};
    my $sum = ($rel{$ind}{$dad} + $rel{$ind}{$mom});
    if ($fev{$dad}) {
	if ($fev{$mom}) {
	    $pred{$ind} = (($rel{$ind}{$dad})*($fev{$dad})/$sum) + (($rel{$ind}{$mom})*($fev{$mom})/$sum);
	    print "$ind\t$dad\t$mom\n";
